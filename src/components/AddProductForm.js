...
function AddProductForm() {
   ...
   // const [product, setProduct] = useState(initProductState);  //กำหนดค่าเริ่มต้นของ state
    ...
    const formik = useFormik({
        ...
        onSubmit:() =>{                    //เมื่อผ่านเงื่อนไข
            if(formik.values.file){      //เช็คว่ามีรูปมารึป่าวถ้าไม่มีไม่ต้องเรียกใช้การอัพโหลดรูป
                console.log("Upload ");
                ...
                {submitted ? (<>
                    ...
                    <FormGroup>
                        ...
                        {progress !=0 && ( 
                                <Progress  animated
                                color="warning"
                                value={75} value={progress} max="100">{progress} %</Progress>
                                        )
                        }
